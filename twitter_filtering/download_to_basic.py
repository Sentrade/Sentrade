...
def process_bz2(tw_json_bz2_filename, local_dir, run_log, year, month, day):
	...
	for keyword in keyword_list:
		search_keyword(keyword, tw_json_filename, local_dir, "{0}-{1}-{2}.json".format(year, str(month).zfill(2), str(day).zfill(2)), run_log)
...
for day in range(1, 32):
	...
	for mem in date_tar.getmembers():
		...
		tw_json_filename = tw_json_bz2_filename.split('.')[0] + ".json"
		process_bz2(tw_json_bz2_filename, local_dir, run_log, year, month, day)
